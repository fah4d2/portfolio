...
export const Route = createFileRoute("/signin")({
  head: () => ({
    meta: [
      { title: "Sign in — Fahad Alazmi" },
      {
        name: "description",
        content: "Sign in to see your meetings with Fahad Alazmi and book faster next time.",
      },
      { property: "og:title", content: "Sign in — Fahad Alazmi" },
      { property: "og:description", content: "Sign in to manage your meetings." },
...
function SignInPage() {
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      router.navigate(
        result.user.role === "admin" ? { to: "/admin" } : { to: "/", hash: "#book" },
      );
    } catch (error) {
        ...
        <Card className="w-full border-border/60 bg-card/60">
          <CardContent className="p-7">
            ...
            <p className="mt-2 text-sm text-muted-foreground">
              Signing in is optional — you can book a meeting as a guest too.
            </p>
